...
function courseAvailability() {
    ...
    $.ajax({
        ...
        data: { cid: $("#course").val(), studentregno: $("#studentregno").val() },
        type: "POST",
        success: function(data) {
            $("#course-availability-status1").html(data);
            var enrollmentStatus = data.includes("Course is available and seats are open.");
            $("#submit").prop("disabled", !enrollmentStatus);
            $("#loaderIcon").hide();
        },
        error: function() {}
    });
